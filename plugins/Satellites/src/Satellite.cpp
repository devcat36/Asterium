...
{
	constexpr float excluded = 50.f;
	constexpr float satelliteBonus = 2.f;

	if (flagVFAltitude && !(minVFAltitude<=height && height<=maxVFAltitude))
		return excluded;

	if (flagVFMagnitude && !(((stdMag<99.) || (RCS>0.))
	                         && (maxVFMagnitude<=getVMagnitude(core))
	                         && (getVMagnitude(core)<=minVFMagnitude)))
		return excluded;
	...
	const float limit = drawer->getFlagStarMagnitudeLimit()
	                    ? static_cast<float>(drawer->getCustomStarMagnitudeLimit())
	                    : drawer->getLimitMagnitude();
	return qMin(getVMagnitude(core)-satelliteBonus, limit);
}
