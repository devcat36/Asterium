...
void Oculars::asteriumReloadEquipment()
{
	settings->sync();
	...
	for (int index = 0; index < ocularCount; index++)
	{
		if (Ocular* item = Ocular::ocularFromSettings(settings, index))
			oculars.append(item);
	}
	const int ccdCount = settings->value("ccd_count", 0).toInt();
	for (int index = 0; index < ccdCount; index++)
	{
		if (CCD* item = CCD::ccdFromSettings(settings, index))
			ccds.append(item);
	}
	const int telescopeCount = settings->value("telescope_count", 0).toInt();
	for (int index = 0; index < telescopeCount; index++)
	{
		if (Telescope* item = Telescope::telescopeFromSettings(settings, index))
			telescopes.append(item);
	}
	const int lensCount = settings->value("lens_count", 0).toInt();
	for (int index = 0; index < lensCount; index++)
	{
		if (Lens* item = Lens::lensFromSettings(settings, index))
			lenses.append(item);
	}

	ready = !oculars.isEmpty();
	maxEyepieceAngle = 0.0;
	determineMaxEyepieceAngle();
	updateLists();
	updateOcularReticle();
