...
#if defined(Q_OS_WIN)
	#define STEL_TOUCH_GESTURES 1
	#include <QPinchGesture>
#endif
#ifdef Q_OS_ANDROID
	#include "gui/StelDialog.hpp"
...
protected:
	...
	{
#if defined(Q_OS_ANDROID)
		if (event->button()==Qt::LeftButton && pressTimer.isValid()
		    && pressTimer.elapsed() > 650
		    && QLineF(event->buttonDownScenePos(Qt::LeftButton), event->scenePos()).length() < 20.)
		{
			...
			QMouseEvent press(QEvent::MouseButtonPress, pos, Qt::RightButton, Qt::RightButton, Qt::NoModifier);
			StelApp::getInstance().handleClick(&press);
			QMouseEvent release(QEvent::MouseButtonRelease, pos, Qt::RightButton, Qt::NoButton, Qt::NoModifier);
...
private:
	...
	void pinchTriggered(QPinchGesture *gesture)
	{
		...
		{
			const qreal total = gesture->totalScaleFactor();
			if (total > 0.)
				StelApp::getInstance().handlePinch(total, gesture->state()==Qt::GestureStarted);
		}
	}
#endif
...
void StelMainView::init()
{
	...
	if (sgui!=Q_NULLPTR)
	{
		QTimer* deferredSave = new QTimer(sgui);
		deferredSave->setSingleShot(true);
		deferredSave->setInterval(1500);
		connect(deferredSave, &QTimer::timeout, this, [sgui]{ sgui->saveSettingsNow(); });
		connect(qApp, &QGuiApplication::applicationStateChanged, this, [sgui, deferredSave](Qt::ApplicationState state)
		{
			deferredSave->stop();
			if (state==Qt::ApplicationSuspended || state==Qt::ApplicationHidden)
				sgui->saveSettingsNow();
...
void StelMainView::endPinch()
{
	...
	if (rootItem)
		rootItem->setAcceptedMouseButtons(Qt::LeftButton | Qt::RightButton | Qt::MiddleButton);
}

bool StelMainView::inTwoFingerGesture() const
{
	return pinchActive || (rootItem && rootItem->acceptedMouseButtons()==Qt::NoButton);
}

void StelMainView::handleTouchForPinch(QObject* obj, QEvent* event)
{
	if (!StelApp::isInitialized())
		return;

	if (!glWidget || !glWidget->window() || obj != glWidget->window()->windowHandle())
		return;
		...
		case QEvent::TouchCancel:
		{
			...
			if (down >= 2)
			{
				...
				for (const auto& p : points)
				{
					if (p.state()==QEventPoint::Released)
						continue;
					if (found==0) a = p.position();
					else if (found==1) { b = p.position(); }
					if (++found==2)
						break;
				}
				const qreal distance = QLineF(a, b).length();

				if (rootItem && rootItem->acceptedMouseButtons()!=Qt::NoButton)
				{
					rootItem->setAcceptedMouseButtons(Qt::NoButton);
				...
				if (!pinchActive)
				{
					if (distance > 20.)
					{
						...
						StelApp::getInstance().handlePinch(1.0, true);
					}
				}
				else if (pinchStartDistance > 0.)
				{
					StelApp::getInstance().handlePinch(distance/pinchStartDistance, false);
					thereWasAnEvent();
				}
			}
