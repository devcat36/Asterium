...
QList<StelDialog*> StelDialog::allDialogs;

namespace
{
QString touchStyleSheet()
{
#if defined(Q_OS_ANDROID)
	return QStringLiteral(
		"\nQCheckBox::indicator, QRadioButton::indicator { width: 24px; height: 24px; }"
		"\nQPushButton, QToolButton { min-height: 34px; padding: 4px 8px; }"
		"\nQLineEdit, QComboBox, QSpinBox, QDoubleSpinBox, QDateTimeEdit { min-height: 56px; }"
		"\nQAbstractSpinBox::up-button, QAbstractSpinBox::down-button { width: 40px; height: 27px; }"
		"\nQComboBox::drop-down { width: 30px; }"
		"\nQTabBar::tab { min-height: 36px; padding: 6px 10px; }"
		"\nQListView::item, QTreeView::item, QTableView::item { min-height: 34px; }"
		"\nQScrollBar:vertical { width: 20px; }"
		"\nQScrollBar:horizontal { height: 20px; }"
		"\nQSlider::handle:horizontal { width: 24px; margin: -8px 0; }");
#else
	return QString();
#endif
}
...
StelDialog* StelDialog::topmostVisible()
{
	QGraphicsScene* scene = StelMainView::getInstance().scene();
	QGraphicsWidget* active = scene ? scene->activeWindow() : nullptr;
	StelDialog* candidate = nullptr;
	for (StelDialog* d : std::as_const(allDialogs))
	{
		if (!d->visible() || !d->proxy || !d->proxy->isVisible())
			continue;
		if (active && d->proxy == active)
			return d;
		candidate = d;
	}
	return candidate;
...
void StelDialog::styleChanged(const QString &style)
{
	...
	if (gui && dialog)
		dialog->setStyleSheet(style + touchStyleSheet());
}
...
#if defined(Q_OS_ANDROID)
void StelDialog::fitProxyToScreen()
{
	if (!proxy || !dialog)
		return;
	const QSize screenSize = StelMainView::getInstance().size();
	const QSize maxSize = 0.95*screenSize;
	if (maxSize.width() <= 0 || maxSize.height() <= 0)
		return;

	const int natural = qMax(dialog->minimumSizeHint().width(), dialog->sizeHint().width());
	double scale = 1.;
	if (natural > maxSize.width() && natural > 0)
	{
		scale = qMax(0.8, static_cast<double>(maxSize.width())/natural);
	}
	proxy->setTransform(QTransform::fromScale(scale, scale));
	const QSize hostSize(qRound(maxSize.width()/scale), qRound(maxSize.height()/scale));
	if (QWidget* host = proxy->widget())
		host->setMaximumSize(hostSize);
	if (androidScrollHost)
		androidScrollHost->setMaximumSize(hostSize);
	proxy->resize(hostSize.width(), hostSize.height());
	proxy->setPos(qRound((screenSize.width() - hostSize.width()*scale) / 2.), 0.);

	if (androidCloseButton)
	{
		androidCloseButton->setMinimumHeight(qRound(48./scale));
	}
}

void StelDialog::refitAllToScreen()
{
	for (StelDialog* d : std::as_const(allDialogs))
	{
		if (d->visible() && d->proxy)
			d->fitProxyToScreen();
	}
}
#endif
...
void StelDialog::setVisible(bool v)
{
	if (v)
	{
		...
		if (dialog)
		{
			...
			if (gui)
				dialog->setStyleSheet(gui->getStelStyle().qtStyleSheet + touchStyleSheet());
			dialog->show();
#if defined(Q_OS_ANDROID)
...
#if defined(Q_OS_ANDROID)
			androidScrollHost = new QScrollArea;
			QScrollArea* scrollHost = androidScrollHost;
			scrollHost->setFrameShape(QFrame::NoFrame);
			scrollHost->setWidgetResizable(true);
			scrollHost->setWidget(dialog);
			scrollHost->setMaximumSize(maxSize);
			scrollHost->setStyleSheet(
				"QAbstractScrollArea { background-color: rgb(16,16,20); }");
			scrollHost->viewport()->setAutoFillBackground(true);
			QScroller::grabGesture(scrollHost->viewport(), QScroller::LeftMouseButtonGesture);

			QWidget* host = new QWidget;
			QVBoxLayout* hostLayout = new QVBoxLayout(host);
			hostLayout->setContentsMargins(0, 0, 0, 0);
			hostLayout->setSpacing(0);
			hostLayout->addWidget(scrollHost, 1);

			androidCloseButton = new QToolButton(host);
			androidCloseButton->setText(q_("Close"));
			androidCloseButton->setToolButtonStyle(Qt::ToolButtonTextOnly);
			androidCloseButton->setFocusPolicy(Qt::NoFocus);
			androidCloseButton->setCursor(Qt::ArrowCursor);
			androidCloseButton->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed);
			androidCloseButton->setMinimumHeight(48);
			androidCloseButton->setStyleSheet(
				"QToolButton { background-color: rgb(46,46,54); color: white;"
				" border: none; border-top: 1px solid rgb(90,90,104);"
				" font-size: 17px; font-weight: bold; }"
				"QToolButton:pressed { background-color: rgb(96,96,112); }");
			connect(androidCloseButton, &QToolButton::clicked, this, &StelDialog::close);
			hostLayout->addWidget(androidCloseButton, 0);

			scrollHost->setStyleSheet(scrollHost->styleSheet() +
				"\nQScrollBar:horizontal { height: 26px; }"
				"\nQScrollBar:vertical { width: 26px; }"
				"\nQScrollBar::handle { background: rgb(130,130,150); border-radius: 5px; min-width: 40px; min-height: 40px; }");
...
#if defined(Q_OS_ANDROID)
			proxy->resize(maxSize.width(), maxSize.height());
			proxy->setPos(qRound((screenSize.width() - maxSize.width()) / 2.), 0.);
			fitProxyToScreen();
#endif
