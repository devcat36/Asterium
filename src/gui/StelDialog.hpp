...
public:
	...
	static StelDialog* topmostVisible();
...
public:
	static void refitAllToScreen();
protected:
	class QToolButton* androidCloseButton = nullptr;
	class QScrollArea* androidScrollHost = nullptr;

	void fitProxyToScreen();
#endif

	static QList<StelDialog*> allDialogs;
...
/*			QStyleOptionTitleBar bar;
	...
	protected:
		...
		{
#if defined(Q_OS_ANDROID)
			constexpr bool transparencyDefault = false;
#else
			constexpr bool transparencyDefault = true;
#endif
			if (StelApp::getInstance().getSettings()->value("gui/flag_use_window_transparency", transparencyDefault).toBool())
			{
				switch (event->type())
				{
