...
void LeftStelBar::setMaxHeight(double h)
{
	if (qFuzzyCompare(maxHeight, h))
		return;
	maxHeight = h;
	prepareGeometryChange();
...
{
	double posX = 0., posY = 0., columnWidth = 0.;
	for (const auto button : childItems())
	{
		const auto b = dynamic_cast<StelButton*>(button);
		if (!b)
			continue;
		b->animValueChanged(0.); // update button pixmap
		const double step = std::round(b->boundingRect().height() + 9.5 * StelButton::buttonSizeRatio());
		if (maxHeight > 0. && posY > 0. && posY + step > maxHeight)
		{
			posX += columnWidth;
			posY = 0.;
			columnWidth = 0.;
		...
		posY += step;
		columnWidth = qMax(columnWidth, std::round(b->boundingRect().width()));
	}
}
...
void BottomStelBar::setMaxWidth(double w)
{
	if (qFuzzyCompare(maxWidth, w))
		return;
	maxWidth = w;
	prepareGeometryChange();
	updateButtonsGroups();
...
void BottomStelBar::updateButtonsGroups()
{
	...
	const double firstRowY = statusFM.lineSpacing()+gap; // Take natural font geometry into account
	double y = firstRowY;
	const double rowSpacing = std::round(2. * StelButton::buttonSizeRatio());
	double rowHeight = 0.;
	for (auto& group : buttonGroups)
	{
		...
		if (maxWidth > 0.)
		{
			double groupWidth = group.leftMargin + group.rightMargin;
			for (const auto* b : buttons)
				groupWidth += b->getButtonPixmapWidth();
		...
		for (auto* b : buttons)
		{
			...
			if (maxWidth > 0. && x > 0. && x + b->getButtonPixmapWidth() > maxWidth)
			{
				x = 0.;
				y += rowHeight + rowSpacing;
				rowHeight = 0.;
			...
			rowHeight = qMax(rowHeight, static_cast<double>(b->getButtonPixmapHeight()));
			++n;
		}
		x+=group.rightMargin;
	}
