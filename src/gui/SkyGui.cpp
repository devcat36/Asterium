...
static constexpr qreal autoHideBtScale = 1.0;
...
void SkyGui::init(StelGui* astelGui)
{
	...
	btHorizAutoHide->setScale(autoHideBtScale);
	btVertAutoHide->setScale(autoHideBtScale);

	btHorizAutoHide->setPos(1,(btVertAutoHide->getButtonPixmapHeight()-btHorizAutoHide->getButtonPixmapHeight())*autoHideBtScale+1);
...
void SkyGui::updateInfoPanelPos()
{
	...
	infoPanel->setPos(left, margin);
	const qreal available = getSkyGuiWidth() - left - margin;
	infoPanel->setTextWidth(available > 0. ? available : -1.);
}
...
void SkyGui::updateBarsPos()
{
	...
	double leftBarReserve = 0.;
	if (ww > 0 && hh > 0)
	{
		const double frame = 2. * buttonBarsFrame->getRoundSize();
		const double heightShare = (ww > hh) ? 0.92 : 0.75;
		leftBar->setMaxHeight(qMax(64., hh * heightShare));
		leftBarReserve = leftBar->boundingRectNoHelpLabel().right();
		if (leftBarReserve > ww * 0.2)
			leftBarReserve = 0.;
		bottomBar->setMaxWidth(qMax(64., ww - leftBarReserve - frame - 4.));
		updateInfoPanelPos();
	}
	...
	const qreal newLeftBarY = qMax(0., static_cast<double>(hh)-leftBar->boundingRectNoHelpLabel().height()-bottomBar->boundingRectNoHelpLabel().height()-20);
	...
	const qreal newBottomBarX = leftBarReserve+buttonBarsFrame->getRoundSize();
	...
	if (autoHidebts)
	{
		...
		const double opacity = qMax(animLeftBarTimeLine->currentValue(), animBottomBarTimeLine->currentValue());
		autoHidebts->setOpacity(qMax(0.01, opacity));	// 0.01 works around a Qt bug
	}
