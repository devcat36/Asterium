...
void StelGui::init(QGraphicsWidget *atopLevelGraphicsWidget)
{
...
#if defined(Q_OS_ANDROID)
	actionsMgr->addAction("actionAsterium_Reset_View", N_("Movement and Selection"),
	                      N_("Reset the view"), this, "resetSkyView()", "", "");
#endif
...
#if defined(Q_OS_ANDROID)
	constexpr bool quitButtonDefault = false;
#else
	constexpr bool quitButtonDefault = true;
#endif
	setFlagShowQuitButton(conf->value("gui/flag_show_quit_button", quitButtonDefault).toBool());
	...
	{
		const QString last = conf->value("asterium/last_observer_location").toString();
		if (!last.isEmpty())
		{
			const StelLocation loc = StelApp::getInstance().getLocationMgr().locationForString(last);
			if (loc.isValid())
...
void StelGui::saveSettingsNow()
{
	if (StelApp::getInstance().getSettings()->value("main/restore_defaults", false).toBool())
		return;

	if (configurationDialog)
		configurationDialog->saveAllSettings(false);
#if defined(Q_OS_ANDROID)
	if (StelCore* core = StelApp::getInstance().getCore())
	{
		const QString here = core->getCurrentLocation().serializeToLine();
		if (!here.isEmpty())
		{
			QSettings* conf = StelApp::getInstance().getSettings();
			conf->setValue("asterium/last_observer_location", here);
			conf->sync();
		}
	}
#endif
