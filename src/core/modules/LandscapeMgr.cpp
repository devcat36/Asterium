...
LandscapeMgr::LandscapeMgr()
	...
	, flagLandscapeAutoTransparency(true)
	, autoTransparency(0.)
...
void LandscapeMgr::update(double deltaTime)
{
	...
	if (flagLandscapeAutoTransparency)
	{
...
void LandscapeMgr::draw(StelCore* core)
{
	...
	// Draw the landscape
	Landscape::setTransparency(getEffectiveLandscapeTransparency());
...
void LandscapeMgr::init()
{
	...
	setFlagLandscapeAutoTransparency(conf->value("landscape/flag_auto_transparency", true).toBool());
...
{
	if (getEffectiveLandscapeTransparency()>0.)
		return -1.;
	else
...
bool LandscapeMgr::getFlagLandscapeAutoTransparency() const
{
	return flagLandscapeAutoTransparency;
}

void LandscapeMgr::setFlagLandscapeAutoTransparency(bool b)
{
	if (b!=flagLandscapeAutoTransparency)
	{
		flagLandscapeAutoTransparency=b;
		StelApp::immediateSave("landscape/flag_auto_transparency", b);
		emit flagLandscapeAutoTransparencyChanged(b);
	}
}

double LandscapeMgr::getEffectiveLandscapeTransparency() const
{
	if (flagLandscapeAutoTransparency)
		return autoTransparency;
	return flagLandscapeUseTransparency ? landscapeTransparency : 0.0;
}
