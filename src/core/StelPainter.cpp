...
float StelPainter::textHaloStrength=0.f;
...
StringTexture* StelPainter::getTextTexture(const QString& str, int pixelSize) const
{
	...
	const int haloPixels = halo ? qMax(2, qRound(0.08f*tmpFont.pixelSize())) : 0;
	if (haloPixels)
		strRect.adjust(-haloPixels, -haloPixels, haloPixels, haloPixels);
	...
	if (haloPixels)
	{
		...
		QPainterPath path;
		path.addText(-strRect.x(), -strRect.y(), tmpFont, str);
		painter.setPen(QPen(QColor(0, 255, 0), 2.*haloPixels,
...
uniform mediump float haloStrength;
void main()
{
	mediump vec2 masks = texture2D(tex, texc).rg;
	mediump float glyph = masks.r;
	mediump float halo = masks.g*haloStrength;
	mediump float cover = max(glyph, halo);
	mediump vec3 rgb = cover > 0.0 ? textColor.rgb*(glyph/cover) : textColor.rgb;
	FRAG_COLOR = vec4(rgb, textColor.a*cover);
})";
	...
	textShaderVars.haloStrength = textShaderProgram->uniformLocation("haloStrength");
