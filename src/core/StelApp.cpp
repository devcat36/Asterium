...
void StelApp::init(QSettings* conf)
{
	...
	constexpr int defaultFontSize = getDefaultGuiFontSize();
	constexpr int defaultDialogFontSize = getDefaultGuiFontSize() + 3;
	constexpr double defaultButtonScale = 150.;
#else
	constexpr int defaultFontSize = getDefaultGuiFontSize();
	constexpr int defaultDialogFontSize = getDefaultGuiFontSize();
	constexpr double defaultButtonScale = 100.;
#endif
	setScreenFontSize(confSettings->value("gui/screen_font_size", defaultFontSize).toInt());
	setGuiFontSize(confSettings->value("gui/gui_font_size", defaultDialogFontSize).toInt());
	setScreenButtonScale(confSettings->value("gui/screen_button_scale", defaultButtonScale).toDouble());
