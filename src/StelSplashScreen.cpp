...
#define ASTERIUM_BRANDED_SPLASH_PIXMAP 1
...
void SplashScreen::present(const double sizeRatio)
{
	...
	for (int i = 0; i < 100 && !instance->hasPainted(); ++i)
	{
...
void SplashScreen::clearMessage()
{
	OpenGLContextPresever contextPreserver;
	Q_ASSERT(instance);
	instance->setStatusMessage(QString());
...
SplashScreen::SplashScreenWidget::SplashScreenWidget(const double sizeRatio)
	...
	, sizeRatio(sizeRatio)
{
	const QPixmap canvas = makePixmap(sizeRatio, QSize());
...
void SplashScreen::SplashScreenWidget::resizeEvent(QResizeEvent* event)
{
...
void SplashScreen::SplashScreenWidget::drawContents(QPainter* painter)
{
...
int SplashScreen::SplashScreenWidget::textPixelSize(const double sizeRatio) const
{
#if defined(Q_OS_ANDROID)
	if (!artRect.isEmpty())
...
QPixmap SplashScreen::SplashScreenWidget::makePixmap(const double sizeRatio, const QSize& target)
{
	QPixmap pixmap(StelFileMgr::findFile("data/splash.png"));
#if defined(Q_OS_ANDROID)
	if (QScreen* screen = QGuiApplication::primaryScreen())
	{
		const QSize screenSize = target.isEmpty() ? screen->size() : target;
		if (!pixmap.isNull() && screenSize.isValid())
		{
			const qreal dpr = qMax(1.0, screen->devicePixelRatio());
			QPixmap canvas(QSize(std::lround(screenSize.width()*dpr),
	...
	artRect = QRect(QPoint(), pixmap.deviceIndependentSize().toSize());
	return pixmap;
}
